...
discord.server.getGuildId = () => {
  const m = location.href.match(/channels\/([\w@]+)\/(\d+)/);
  if (location.href.match(/channels\/([\w@]+)\/(\d+)/)[1] === '@me') return location.href.match(
    /channels\/([\w@]+)\/(\d+)/)[2];
  if (m) return m[1];
  else console.log('Could not the Guild ID!\nPlease make sure you are on a Server or DM.');
}

discord.server.getChannelId = () => {
  const m = location.href.match(/channels\/([\w@]+)\/(\d+)/);
  if (m) return m[2];
  else console.log('Could not the Channel ID!\nPlease make sure you are on a Channel or DM.');
}
